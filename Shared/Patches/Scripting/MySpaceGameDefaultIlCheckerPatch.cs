...
public static class MySpaceGameDefaultIlCheckerPatch
{
    ...
    private static bool AllDeclaredMembersPrefix(Type type, ref IEnumerable<MemberInfo> __result)
    {
        ...
        __result = members.Where(
            (Func<MemberInfo, bool>)(
                m => m is not Type && !MySpaceGameDefaultIlChecker.IsPropertyMethod(m)
            )
        );
        return false;
    }
    ...
    private static bool AllowDefaultNamespacesPrefix(IMyWhitelistBatch handle)
    {
        ...
        handle.AllowTypes(
            MyWhitelistTarget.Ingame,
            (
                from x in typeof(Enumerable).Assembly.GetTypes()
                where x.Namespace == "System.Linq"
                where
                    !x.Name.StringExtensions_Contains(
                        "parallel",
        ...
        handle.AllowTypes(
            ...
            typeof(XmlArrayItemAttributes),
            typeof(XmlAttributeEventArgs),
            typeof(XmlAttributeOverrides),
            typeof(XmlAttributes),
            typeof(XmlChoiceIdentifierAttribute),
            typeof(XmlElementAttributes),
            typeof(XmlElementEventArgs),
        ...
        handle.AllowMembers(
            MyWhitelistTarget.Both,
            typeof(Environment).GetProperty(
        ...
        handle.AllowMembers(
            MyWhitelistTarget.Both,
            type.GetMethod("GetFields", [typeof(BindingFlags)])
        );
        handle.AllowMembers(
            MyWhitelistTarget.Both,
            (
                from m in MySpaceGameDefaultIlChecker.AllDeclaredMembers(typeof(Delegate))
    ...
    public static bool StringExtensions_Contains(
        this string text,
        string testSequence,
