...
public static class MyScriptCompilerPatch
{
        ...
        List<MetadataReference> ___m_metadataReferences)
    {
        ...
        foreach (var assemblyLocation in assemblyLocations)
        {
            ...
            {
                Console.WriteLine($"{ServerPlugin.Plugin.Name} [WARNING] AddReferencedAssembliesPrefix: Empty assembly location {assemblyLocation}");
#if DEBUG
                Debugger.Break();
#endif
                continue;
    ...
    public static bool AddImplicitInGameNamespacesFromTypesPrefix(Type[] types, HashSet<string> ___m_implicitScriptNamespaces)
    {
        ...
        foreach (var type in types)
        {
            ...
            {
                Console.WriteLine($"{ServerPlugin.Plugin.Name} [WARNING] AddImplicitInGameNamespacesFromTypesPrefix: Empty namespace name {type.Namespace}");
#if DEBUG
                Debugger.Break();
#endif
                continue;
