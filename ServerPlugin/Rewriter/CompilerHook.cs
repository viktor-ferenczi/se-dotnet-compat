...
namespace ServerPlugin.Rewriter
{
    ...
    /// any mod (the patched <c>CreateCompilation</c> is the consumer). On the
    /// dedicated server <c>IPlugin.Init</c> runs <em>after</em> the auto-loaded
    /// session has already compiled mods, so consumers must register from
    /// their preloader's <c>Finish()</c> instead — which runs before any
    /// session loads. The hook patch itself is in the <c>"Finish"</c> Harmony
