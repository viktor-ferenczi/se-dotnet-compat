...
internal class MyTextureData : MyImmediateRC
{
	internal static bool ToFile(IResource res, string path, MyImage.FileFormat fmt)
	{
		try
		{
			Directory.CreateDirectory(Path.GetDirectoryName(path) ?? throw new InvalidOperationException($"Path has no directory component: {path}"));
	...
	{
		var texture2D = res.Resource as Texture2D ?? throw new InvalidOperationException($"Resource is not a Texture2D: {res.Resource.GetType()}");
		var format = texture2D.Description.Format;
		var texture2D2 = new Texture2D(MyRender11.DeviceInstance, new Texture2DDescription
		{
		...
		RC.CopyResource(res, texture2D2);
		DataStream stream2;
		DataBox dataBox = RC.MapSubresource(texture2D2, 0, 0, MapMode.Read, SharpDX.Direct3D11.MapFlags.None, out stream2);
		Save(format, stream, fileFormat, stream2.DataPointer, dataBox.RowPitch, res.Size);
		RC.UnmapSubresource(texture2D2, 0);
		texture2D2.Dispose();
	}
	...
	{
		// Use MyImage.Save method directly
		switch (format)
		{
